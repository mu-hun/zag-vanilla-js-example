...
export class Combobox extends Component<combobox.Props, combobox.Api> {
  ...
  render = () => {
    ...
    const triggerEl =
      this.rootEl?.querySelector<HTMLElement>('.combobox-trigger')
    if (triggerEl) spreadProps(triggerEl, this.api.getTriggerProps())

    const clearTriggerEl = this.rootEl?.querySelector<HTMLElement>(
    ...
    const controlEl =
      this.rootEl?.querySelector<HTMLElement>('.combobox-control')
